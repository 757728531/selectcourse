				...
				<c:if test="${sublistNum>0}">
					<c:forEach items="${userSubListAll}" var="usa">
						<tr>
							...
							<td><a href="lookAskBook.do?id=${usa.id}">查看申请内容</a></td>
							<td>
								<c:if test="${usa.subscribeStatus==0}"> 正在审核</c:if>
								<c:if test="${usa.subscribeStatus==1}"> 审核通过</c:if>
								<c:if test="${usa.subscribeStatus==2}"> 审核未通过</c:if>
							</td>
							<c:if test="${usa.subscribeStatus=='0'}">
								<td><%-- <a href="deleteSubAsk.html?id=${usa.id}">撤销申请</a> --%>
									<a href="deleteSubAsk.do?id=${usa.id}" onclick="if(confirm('确定撤销申请?')==false)return false;">
