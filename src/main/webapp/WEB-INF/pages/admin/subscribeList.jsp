			...
			<table class="table table-hover table-striped">
				...
				<c:forEach items="${subscribeList}" var="sl">
					<tr>
						...
						<td><a href="subScribeContent.do?id=${sl.id}">查看</a></td>
						<td>
							<c:if test="${sl.subscribeDate.time<now.time}">
								已失效 | <a href="deleteSubscribeYes.do?id=${sl.id}" onclick="if(confirm('确定清除该条记录？')==false) return false;">清除</a>
							</c:if>
							<c:if test="${sl.subscribeDate.time>now.time}">
								<c:if test="${sl.subscribeStatus==0}"> 正在审核</c:if>
								<c:if test="${sl.subscribeStatus==1}"> 审核通过</c:if>
								<c:if test="${sl.subscribeStatus==2}"> 审核未通过</c:if>
							</c:if>
						</td>
					</tr>
				</c:forEach>
