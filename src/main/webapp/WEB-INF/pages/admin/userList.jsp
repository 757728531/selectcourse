		...
		<div class="personal col-sm-9 col-xs-12">
			<table class="table table-hover table-striped">
				...
				<c:forEach items="${userList}" var="um">
					<tr>
						<td><a href="${um.id}/onlyUser.do">${um.userName}</a></td>
						<td>${um.userId}</td>
						<td>
						...
						<td>${um.userOther}</td>
						<td>
							<c:if test="${um.userStatus==1}">通过审核</c:if>
							<c:if test="${um.userStatus==2}">待审核</c:if>
							<c:if test="${um.userStatus==3}">审核被拒绝</c:if>
						</td>
						<td><a href="deleteUser.do?id=${um.id}&page=${IntPage}" onclick="if(confirm('确定删除该用户?')==false)return false;"><button class="btn btn-warning">删除</button></a></td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
			...
			<ul class="pagination">
            	...
            	<li class="disabled"><a href="javascript:void(0)">&laquo;</a></li>      
 				...
 				<c:if test="${IntPage!=1}">
				<li><a href="userList.do?page=${IntPage-1}">&laquo;</a></li>
				</c:if>
				<c:forEach begin="1" end="${pageNum}" varStatus="status">
					...
					<c:if test="${IntPage!=status.index}">
						<li class=""><a href="userList.do?page=${status.index}">${status.index}</a></li>
					</c:if>
				</c:forEach>
				<c:if test="${IntPage<pageNum}">
				<li><a href="userList.do?page=${IntPage+1}">&raquo;</a></li>
				</c:if>
