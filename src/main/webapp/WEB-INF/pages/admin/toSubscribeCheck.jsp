			...
			<table class="table table-hover table-striped">
				...
				<c:forEach items="${subCheckList}" var="sl">
					<tr>
						...
						<td><a href="subScribeContent.do?id=${sl.id}">申请原因</a></td>
						<td>${sl.subscribeStatus}</td>
						<td><a onclick="if(confirm('确定通过该用户请求？')==false) return false;" href="updateSubscribe.do?id=${sl.id}">通过</a> | <a onclick="if(confirm('确定要拒绝该用户请求？')==false) return false;" href="updateSubscribeNo.do?id=${sl.id}">拒绝</a></td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
